    ...
    async def rankedwatch_loop(self):
        async with aiosqlite.connect("./database/main.db") as db:
            async with db.cursor() as cursor:
                ...
                await cursor.execute('UPDATE players SET tier = (SELECT updated.tier FROM updated where updated.summonerName = summonerName), division = (SELECT updated.division FROM updated where updated.summonerName = summonerName), LP = (SELECT updated.LP FROM updated where updated.summonerName = summonerName), lastmatch = (SELECT updated.lastmatch FROM updated where updated.summonerName = summonerName)')
                await db.commit()
